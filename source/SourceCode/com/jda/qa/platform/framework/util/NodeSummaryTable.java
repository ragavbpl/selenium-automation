...
public class NodeSummaryTable implements TableInterface, InlineEditTableInterface {
...
//Variables
	...
	String sNodePool_Actions_Location;
	String selectCellDD;
...
//Collection Arrays
	...
	/* constructor for NodeSummaryTable
	 * */
	public NodeSummaryTable (TestBrowser oBrowser, String sTableProps,
	...
	public NodeSummaryTable (TestBrowser oBrowser, String sTableProps,
		...
		sNodePool_Actions_Location=pTableProperties.getProperty("sNodePool_Actions_Location");
		selectCellDD=pTableProperties.getProperty("selectCellDD");		
		...
		for (int i = 1; i < iCount; i++) {
			((JavascriptExecutor) driver).executeScript(
					...
					driver.findElement(By.xpath(sTable_ColsHeader_Location
							+ "[" + (i + 1) + "]")));
			oColHdrNames.put(oBrowser.getText(sTable_ColsHeader_Location + "[" + (i + 1) + "]").trim().replace("&nbsp;", " "), i - 1);
			oColHeaders.add(oBrowser.getText(sTable_ColsHeader_Location + "[" + (i + 1) + "]").trim().replace("&nbsp;", " "));
		}		
		return oColHdrNames;
	}
	...
	public void selectTableRow(){
		...
		if (iRow == -1) {
			...
			((JavascriptExecutor) driver).executeScript(
					"arguments[0].scrollIntoView();",
					driver.findElement(By.xpath(sNodePool_Actions_Location + "/tr["
							+ (iRow + 1) + "]/td[2]")));

			oBrowser.click(sNodePool_Actions_Location + "/tr[" + (iRow + 1)
					+ "]/td[2]");				
		}
	}
	...
	public int getTableRowNumber(String pkColValue) {
		...
		for(int i=0;i<pkColValue.split(",").length;i++)
		{
			// santosh commented colforSelection.put(Integer.toString(i),objectDetails.get(pkColValue.split(",")[i]));
	...
	public String getTableCellContent(int rowNum, String colHdrName) {
		...
		colContentLoc = sTable_Content_RowLocation + "[" + rowNum + "]" + "/td[contains(@class,'gridData')]["
				+ getTableColumnNumber(colHdrName) + "]";

		if (oBrowser.isElementPresent(
				colContentLoc + "/input[@type='checkbox']", true)) {
			boolean checkedStatus = oBrowser.isChecked(colContentLoc
					+ "/input[@type='checkbox']");
			...
			return oBrowser.getText(colContentLoc);
		}
	...
	public void setTableCellContent(int rowNum, String colHdrName,
		...
		if (!colContent.equalsIgnoreCase("SKIP")) {
			...
			colContentLoc = sTable_Content_RowLocation + "[" + (rowNum+1) + "]" + "//td["
					+ (getTableColumnNumber(colHdrName) +1) + "]";
				...
				}else if (oBrowser.isElementPresent(selectCellDD, true)) {
					oBrowser.select(selectCellDD, colContent);
				      WebElement selectCellDDCLK = driver.findElement(By.xpath(selectCellDD));
				      selectCellDDCLK.sendKeys(Keys.ENTER);
