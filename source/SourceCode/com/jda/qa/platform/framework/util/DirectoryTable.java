...
public class DirectoryTable implements TableInterface {
	...
	public DirectoryTable(TestBrowser browser, String tableProps,
	...
	public void loadTable() {
		...
		tableLoc = "//*[@id='tblDirectoryMgrGrid']";
		tableRowCountLoc = tableLoc + "/tbody/tr";
		...
		// Expand the Root entries

		String rootExpandLoc = "//*[@id='tblDirectoryMgrGrid']/tbody/descendant::a[contains(@onclick,'expandAll')]/img[contains(@src,'grid_expand.gif')]";

		browser.moveToElement(rootExpandLoc);
		
		browser.click(rootExpandLoc);	
		
		browser.sleep(1500);
	
		String expandIconLoc = "//*[@id='tblDirectoryMgrGrid']/tbody/descendant::a[contains(@href,'toggleTreeNode')]/img[contains(@src,'grid_expand.gif')]";

		while (browser.isElementPresent(expandIconLoc, true)) {

			List<WebElement> expandIcons = browser.findElements(expandIconLoc);

			Iterator<WebElement> elems = expandIcons.iterator();

			while (elems.hasNext()) {
				WebElement webElement = (WebElement) elems.next();
				
				browser.moveToElement(webElement);
				...
				if (elems.hasNext()) {
					elems = browser.findElements(expandIconLoc).iterator();
				}
